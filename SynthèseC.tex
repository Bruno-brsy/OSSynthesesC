...
\begin{description}
  ...
  \begin{lstlisting}
    "Hello world!"
  \end{lstlisting}
  %https://openclassrooms.com/courses/apprenez-a-programmer-en-c/les-chaines-de-caracteres -> ici il mette que c'est \0 le caactère de fin de chaine.
  La représentation interne d'une chaîne de caractères est spécifiée. C'est un tableau de char terminé par $\0$, le caractère de fin de chaine.
  \begin{lstlisting}
    char s[] = "Hello";
  \end{lstlisting}
...
Cette fonction permet de réveiller tous les threads qui surveillent la condition cond.
Tout comme pthread\textunderscore{}cond\textunderscore{}signal, elle prend en paramètre l'adresse de la variable-condition surveillée.
\begin{lstlisting}
